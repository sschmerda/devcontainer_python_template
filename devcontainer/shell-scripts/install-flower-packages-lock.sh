...
FALLBACK_ENV="/tmp/flower-environment/flower-environment.yml"

if [ ! -f "$LOCK_FILE" ]; then
  if [ -f "$FALLBACK_ENV" ]; then
    echo "Flower lockfile not found: $LOCK_FILE"
    echo "Falling back to latest env: $FALLBACK_ENV"
    run_with_retries micromamba env create -y -n celery-env -f "$FALLBACK_ENV"
...
echo ">>> Flower micromamba lock installation completed."
