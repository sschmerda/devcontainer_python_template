...
if ! command -v conda-lock >/dev/null 2>&1; then
  ...
  micromamba run -n locktools conda-lock lock -f flower-environment.yml --platform linux-64 --platform linux-aarch64 --lockfile flower-environment-lock.yml
  micromamba env remove -n locktools -y
  micromamba clean --all --yes >/dev/null 2>&1 || true
else
  conda-lock lock -f flower-environment.yml --platform linux-64 --platform linux-aarch64 --lockfile flower-environment-lock.yml
fi
